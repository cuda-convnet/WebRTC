package com.webrtc.auto.manager;
...
import com.webrtc.auto.dao.EngroupDAO;
...
public class EngroupManager {
	...
	public static Engroup updateEngroup2(Integer fz, Integer eid, String fzmc, Integer lfz) {
		Engroup engroup = engroupDAO.findById(fz);
		engroup.setFz(fz);
		engroup.setEid(eid);
		engroup.setFzmc(fzmc);
	 ...
	 * 查询lfz组下的所有分组编号及其名称
	 */
	public static List<Engroup> findByEidLfzForPage(Integer eid,final Object lfz ,int page ,int size)
	{
		try{
			return engroupDAO.findByEidLfzForPage(eid, lfz ,page-1 ,size);
		}catch (Exception e) {
			LOGGER.debug(e);
			return Collections.emptyList();
		}
	 ...
	 * 查询lfz组下的分组总数目
	 */
	public static int fzSize(final Object lfz){
		List<Engroup> engroups = engroupDAO.findByLfz(lfz);
		int size = engroups.size();
