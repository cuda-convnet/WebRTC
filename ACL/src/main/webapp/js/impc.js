...
//							}
	...
	var login = function() {
		...
		var user = formatToEase(userName);
	...
	var buildContactDiv = function(contactlistDivId, roster) {
		...
		for (i = 0; i < roster.length; i++) {
			...
			console.log("执行buildContactDiv，加入一个我的好友！");
			...
			var MenuList2 = $("<li>查看资料</li><li>消息记录</li>");
...
//		/**** bottom_operation ****/
//		var bottom_operation = $("<div class=\"group_operation\"><a href=\"#\">查找</a>|<a href=\"#\">消息盒子</a>|<a href=\"#\">系统设置</a></div>");
//		/**** bottom_operation end ****/
...
//	};
	...
	var handleTextMessage = function(message) {
		...
		console.log("mestype="+mestype);
		console.log("messageContent="+messageContent);

		console.log("OnhandleReceivedTextMessage");
		...
		conn.listRooms({
			success : function(rooms) {
				if (rooms) {
					...
					console.log("handleInviteMessage test1.1");
					for (i = 0; i < rooms.length; i++) {
						var roomsName = rooms[i].name;
						var roomId = rooms[i].roomId;
...
//							$('#list_group_UL').append(lielem);
	...
	var appendMsg2 = function(who, Imcontact, message, chattype) {
		...
		addMessageMsg(who,Imcontact,message);
	}

	var appendMsg3 = function(who, Imcontact, message, chattype) {

		addMessageMsg(Imcontact,Imcontact,message);
