(function () {
    sfclient = function () {
        ...
        peer5.setLogLevel(com.webrtc.config.LOG_LEVEL);

        //monitor the sendQueues
        this.cron_interval_id = window.setInterval(this.cron, com.webrtc.config.MONITOR_INTERVAL, this);
    };
...
//            }
        ...
        addChunks:function (fileName, binarySlice, cb) {
            var blockMap = com.webrtc.file.BlockCache.get(fileName);
            this.numOfChunksInSlice = Math.ceil(binarySlice.byteLength / com.webrtc.config.CHUNK_SIZE);
            for (var i = 0; i < this.numOfChunksInSlice; i++) {
                var start = i * com.webrtc.config.CHUNK_SIZE;
                var newChunk = new Uint8Array(binarySlice.slice(start, Math.min(start + com.webrtc.config.CHUNK_SIZE, binarySlice.byteLength)));
        ...
        calculateStats:function () {
            ...
            if (currentTime - this.lastStatCalcTime < com.webrtc.config.STAT_CALC_INTERVAL) return;
...
//            }
        ...
        sendReport:function (completedDownload) {
            ...
            var thi$ = this;
            var currentTime = Date.now();
            if (currentTime - thi$.lastReportTime < com.webrtc.config.REPORT_INTERVAL && !completedDownload) return;
        ...
        registerEvents:function () {
            ...
            radio('resourceInit').subscribe([function (fileInfo) {
                if (fileInfo) {
                    if(!com.webrtc.config.USE_FS && fileInfo.size > com.webrtc.config.ALLOWED_FILE_SIZE){
                        var maxFileSize = com.webrtc.config.ALLOWED_FILE_SIZE/(1024*1024);
