	...
	<table>
		...
		<tr>
			...
			<td><input name="zw" id="zw" class="squareinput bottommargin_10"
				value="${param.zw}" /> <span id="error7" class="redletter"></span></td>
		</tr>
		<tr>
			<td>部门：</td>
			<td><input name="bm" id="bm" class="squareinput bottommargin_10"
				value="${param.bm}" /> <span id="error8" class="redletter"></span></td>
		</tr>
		<tr>
			<td>办公电话：</td>
			<td><input name="telp" id="telp" class="squareinput bottommargin_10"
				value="${param.telp}" /> <span id="error9" class="redletter"></span></td>
		</tr>
		<tr>
			<td>手机：</td>
			<td><input name="mobp" id="mobp" class="squareinput bottommargin_10"
				value="${param.mobp}" /> <span id="error10" class="redletter"></span></td>
		</tr>
		<tr>
			<td>Email地址：</td>
			<td><input name="emails" id="emails" class="squareinput bottommargin_10"
				value="${param.emails}" /> <span id="error11" class="redletter"></span></td>
		</tr>
		<tr>
			<td>通信地址：</td>
			<td><input name="address" id="address" class="squareinput bottommargin_10"
				value="${param.address}" /> <span id="error12" class="redletter"></span></td>
		</tr>
		<tr>
			<td>邮政编码：</td>
			<td><input name="postcode" id="postcode" class="squareinput bottommargin_10"
				value="${param.postcode}" /> <span id="error13" class="redletter"></span></td>
		</tr>
		<tr>

			<td><input name="fz" id="fz" type="hidden"
				class="squareinput bottommargin_10" value="${param.fz}" /> <span
				id="error14" class="redletter"></span></td>
		</tr>
		<tr>
			<td>备注：</td>
			<td><input name="bz" id="bz" class="squareinput bottommargin_10"
				value="${param.bz}" /> <span id="error15" class="redletter"></span></td>
		</tr>
		<tr>
			<td></td>
			<td class="exit"><br />
				 <input type="button" class="button" value="确定" onclick="modfriend(${param.id})"/> <!-- <button class="graybutton" onclick="return false;">取消</button> -->
				<a href="contactship/contacts" class="graybutton">取消</a></td>
		</tr>
	</table>
<script type="text/javascript">
	function modfriend(id){
		...
		$.ajax({
			...
			success : function(data)
			{
				...
				//	hideLoading();
	        		...
	        		alert("修改联系人成功");
	        		$("#contactlist #tr"+data.id+" td:not(:last)").remove();
	        		$("#contactlist #tr"+data.id).prepend("<td>"+data.lname+"</td><td>"+data.fname+"</td><td>"+data.sex+"</td><td>"+data.nc+"</td><td>"+data.birth+"</td><td>"+data.zw+"</td><td>"+data.bm+"</td><td>"+data.telp+"</td><td>"+data.mobp+"</td><td>"+data.emails+"</td><td>"+data.address+"</td><td>"+data.postcode+"</td><td>"+data.bz+"</td>");
				//}
			},
