...
public class FindMeetingHistory extends ActionSupport{
	private static final Logger logger = Logger.getLogger(FindMeetingHistory.class);
		...
		public String execute() throws Exception{	
			...
			groups = ReservationConfManager.findMeetingHistoryByUsername(username,page-1,size);
			if(groups == null){
				return result;
			}	
			//返回历史会议总数
			count = (int)ReservationConfManager.countMeetingHistoryByUsername(username);
			if(count == -1){
				return result;
			}	
			if(count <=size){
