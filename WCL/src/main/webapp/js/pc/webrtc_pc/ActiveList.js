...
//创建活动会话列表标签
var createActiveList = function(chatFriendId,type,chatGroupId) {
	...
	if (ActivelistDiv == null) { // 没有该项Activelist，则新建
		...
		else if(type == GroupPrefix)
		{

			ActivelistDiv = $("<li num = '0'><div class='list_portrait'>"+
					  "<img src='css/pc/images/img/portrait65_4.jpg' /></div>" +
					  "<a onclick='chooseShowDivType(\"" 
			          + chatGroupId + "\",\"" + type + "\");'>群组 " + chatFriendId + "</a><i class='num' style='display:none;'></i></li>");
			//add by yck
			ActivelistDiv.attr("id", chatGroupId + type + "activelist");
			//ActivelistDiv.attr("id", chatFriendId + type +chatGroupId+ "activelist");
//			ActivelistDiv = $("<a class=\"list-group-item\" onclick='chooseShowDivType(\"" 
//			          + chatGroupId + "\",\"" + type + "\");'>" + chatFriendId + " 群组</a>");
		}
		//创建视频会议活动列表(chatFriendId为roomid)
		else if(type == MeetingPrefix)
		{
			ActivelistDiv = $("<li><div class='list_portrait'>"+
					  "<img src='css/pc/images/img/portrait65_3.jpg'/></div>" +
					  "<a onclick='ShowMeetingDiv(\"" 
			          + chatFriendId + "\",\"" + type + "\");'>视频会议" + chatFriendId + "</a></li>");
			ActivelistDiv.attr("id", chatFriendId + type + "activelist");
		}
	...
	//toggleListMenu("rtc_menu","recent_calls",ActivelistDiv);//高亮最近联系人列表
};
...
var chooseShowDivType = function(FriendId,type){
	...
	if(type==GroupPrefix)//活动会话列表选择显示隐藏群组
	{
		GroupId = FriendId;
		GroupName = GroupId.split("+")[0];
		if(globalCur!=null)
		{
			$(document.getElementById(globalCur)).hide();
		}
		globalCur=type + GroupId;
		curRoomId=GroupId.split("+")[1];
		console.log("通过活动列表打开群组窗口：curRoomId="+curRoomId);
		$(document.getElementById(type + GroupId)).show();

		toggleListMenu("rtc_menu","recent_calls",getActiveList(GroupId,type));//高亮最近联系人列表
	}
	else //活动会话列表选择显示隐藏聊天界面 
	{		
		if(globalCur!=null)
		{
			$(document.getElementById(globalCur)).hide();
