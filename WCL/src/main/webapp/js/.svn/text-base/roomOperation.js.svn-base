...
//创建讨论组
function createDiscussionFn(){
	var discussionName=$('#discusssionname').val();
	var discussionJID= discussionName+"@conference."+com.user.domain;
	var nickName=com.user.email_id;
	com.xmpp.createInstantRoom(discussionJID,nickName);
}
//发起聊天室会话
...
function deleteRoomFn(roomName){
	...
	var rooJID = com.xmpp.leaveRoom(roomJID);
...
function getRoomMember(roomname){
    ...
    var roomlistside = [];
    for(var i=0; i<roommemberlist.length; i++){
  	   if(roommemberlist[i].affiliation==="Owner")
    	roomlistside[i] = "<div class='leftfloat padding715 dottedline owner'>"+roommemberlist[i].jid+"</div>";
  	   else if(roommemberlist[i].affiliation==="Admin")
  	    	roomlistside[i] = "<div class='leftfloat padding715 dottedline admin' >"+roommemberlist[i].jid+"</div>";
  	   else
  	    	roomlistside[i] = "<div class='leftfloat padding715 dottedline'>"+roommemberlist[i].jid+"</div>";

    }
  var roomlistsum = roomlistside.join("");
  console.log(roomlistsum);
  $('.owner').prepend("<image src='images/Owner.jpg'></image>");
  $('#list').append(roomlistsum);
}
...
function loadlistDiv(roomObj, roomListDiv){
	...
	var memberListWidth = 200, memberListHeight = 230;
	var memberListDiv = document.createElement("div");
	roomListDiv.appendChild(memberListDiv);
	memberListDiv.id="list";
	$(memberListDiv).css({'width':memberListWidth+'px', 'height':memberListHeight+'px', 
		'background':'#ffffff', 'color':'#000000', 'margin':'0 5px 5px'});
	$(memberListDiv).html('<list width="'+memberListWidth+'" height="'+memberListHeight+'" id="'+roomObj+'" autoplay="autoplay"></video>');
    getRoomMember(roomObj);
	var operationDiv=document.createElement("div");
    var operationWidth = 200;
	roomListDiv.appendChild(operationDiv);
	var operationId="operation_"+roomObj;
	$(operationDiv).attr('id',operationId);
	$(operationDiv).css({'width': operationWidth});
	$(operationDiv).attr('class','videoarea centeralign leftmargin_5 bottommargin_5');
	$(operationDiv).append('<a title="发起视频通话" onclick="beginRoomVideo(\''+roomJID+'\')"><img src="images/roomVideo.jpg" class=" greybutton"/></a>');	
	$(operationDiv).append('<a title="发起音频通话" onclick="beginRoomAudio(\''+roomJID+'\')"><img src="images/roomAudio.jpg" class=" greybutton"/></a>');
	$(operationDiv).append('<a title=\"新建讨论组\" href=\"basic/_createDiscussion.jsp\" rel=\"facebox\" size=\"s\"><img src="images/discussion.jpg" class="greybutton"/></a>');
	$(operationDiv).append('<a title="文件传输" onclick="createDiscussionFn()"><img src="images/fileTransfer.jpg" class="greybutton"/></a>');
//	var roomJID = roomobj+"@conference.webrtc";
//	var listDivWidth = 200, listDivHeight = 310;
//	var listId = 'list'+roomobj;
//	var listDiv = document.createElement("div");
//	$(listDiv).attr('id',listId);
...
//	roomlistDiv.appendChild(operationDiv);
//	var operationId="operation_"+roomobj;
//	$(operationDiv).attr('class','listarea centeralign leftmargin_5 bottommargin_5');
//	$(operationDiv).attr('id',operationId);
//	$(operationDiv).html('');
//	$(operationDiv).append('<a title="发起视频通话" onclick="beginRoomVideo(\''+roomJID+'\')"><img src="images/video.png" class="video greybutton"/></a>');
//	$(operationDiv).append('<a title="群成员列表" onclick="getroommember(\''+roomobj+'\')"><input type="button" class="button" value="群成员列表"/></a>');		
//	$(operationDiv).append('<onclick="roomJoinInviteFn()"><input type="button" class="button" value="邀请他人"/>');
//	$(operationDiv).append('<a title="新建讨论组" onclick="createDiscussionFn()"><img src="images/discussion.jpg" class="greybutton"/></a>');
}
//邀请他人
...
//加载多人视频的标题栏
function loadvideoTitleDiv(reallroom, roomdialogDiv, videoWidth){
	var titleHeight = 30, titleTableHeight = 24;
	var roomtitleDiv=document.createElement('div');		//标题Div
	roomdialogDiv.appendChild(roomtitleDiv);
	$(roomtitleDiv).css({'width':videoWidth+'px', 'height':titleHeight+'px', 'color':'#333333'});
	
	var roomtitleTable=document.createElement("table");
	roomtitleDiv.appendChild(roomtitleTable);
	$(roomtitleTable).css({'width':videoWidth+'px', 'height':titleTableHeight+'px', 'margin':'2px 5px'});
	...
	var roomtitleTd2=document.createElement("td");
	roomtitleTr.appendChild(roomtitleTd2);
	var roomtitleTd2Width = videoWidth - 30 - 25;
	$(roomtitleTd2).css({'width':roomtitleTd2Width+'px', 'text-align':'left'});
	//edit by qiepei 添加“在线”状态
	var roomtitleTd2Html= $('#'+reallroom+' div:first').html(); 
	if(roomtitleTd2Html == '' || roomtitleTd2Html == undefined){
...
var count=2;
var videoHeight = 400, videoWidth = 800;
var t = null, l = null;
function resetPosition(){
	var h = $(window).height(), w = $(window).width();
	if(t == null && l == null){
...
function createRoomvideoDiv(realroom, isNew){ 
	...
	if(document.getElementById(roomdialogID)){          	//检测对话框是否应经存在
		...
		document.body.appendChild(roomdialogDiv);
		roomdialogDiv.id=roomdialogID;                  		//给层dialogDiv设置属性
		$(roomdialogDiv).css({'width':videoWidth+'px', 'height':videoHeight+'px'});
		$(roomdialogDiv).attr('class', 'greybox dialogbox');
		$(roomdialogDiv).css({'position':'absolute', 'top':t+'px', 'left':l+'px'});
		$(roomdialogDiv).attr('onselectstart', 'return false');
		$(roomdialogDiv).attr('onselect', 'document.selection.empty()');
		...
		loadvideoTitleDiv(realroom, roomdialogDiv, videoWidth);
		var imDiv = document.createElement("div");
		roomdialogDiv.appendChild(imDiv);
		$(imDiv).attr('class', 'leftfloat');
...
//右侧加载多人视频会话
function loadvideoDiv(roomobj, roomlistDiv){
	//roomobj means roomBareJID without@conference.webrtc
	console.log(roomobj,roomlistDiv);
	var smallWidth = 130, smallHeight = 100;
	var videoDivWidth = 400, videoDivHeight = 310;
	var videoId = 'video'+roomobj;
	var videoDiv = document.createElement("div");
	var roomJID = roomobj+"@conference.webrtc";
	var userList=[];
	var localUserId = $('.pub_banner').attr("userId");
	var operationId='operation'+localUserId+'_'+roomJID;
	userList=com.groupChat.getRoomMemberList(roomJID);
	roomlistDiv.appendChild(videoDiv);
	videoDiv.id=videoId;
	$(videoDiv).css({'width':videoDivWidth+'px', 'height':videoDivHeight+'px', 
		 'margin':'0 5px 5px'});
	$(videoDiv).css({'background':'#ffffff', 'color':'#000000', 'overflow':'auto'});
	...
	$(operationDiv).attr('id',operationId);
	$(operationDiv).html('');
	$(operationDiv).append('<a title="发起多人视频会话" onclick="beginRoomVideo(\''+roomJID+'\')"><img src="images/video.png" class="video greybutton"/></a>');		
	$(operationDiv).append('<a title="发起多人音频会话" onclick="beginRoomAudio(\''+roomJID+'\')"><img src="images/voice.png" class="audio greybutton"/></a>');	
	$(operationDiv).append('<a title="挂断当前通话" onclick="endRoomVideo(\''+roomJID+'\')"><img src="images/hangup.png" class="hidden hang greybutton"/></a>');
	
//	var roomMemberID = [];
//	roomMemberID = com.xmpp.getOnlineRoomMemberList(roomJID);
//	for(var i=0;i<roomMemberID.length;i++){
//		var VideoId='video_'+roomMemberID[i];
//	}
	var VideoId1 = 'video_'+roomobj;
	var VideoId2 = 'video_jxk143@163.com';
	var VideoId3 = 'video_woshikelebaobao@163.com';
	var VideoId4 = 'video_cadobe@qq.com';
	var VideoId5 = 'video_woshikelebaobao@163.com';
	var VideoId6 = 'video_shanxuanbupt@qq.com';
	var VideoId7 = 'video_caixin0219@qq.com';
	...
	$(VideoDIV1).css({'width':videoDivWidth+'px','height':smallHeight+'px'});
	$(VideoDIV2).css({'width':videoDivWidth+'px','height':smallHeight+'px'});
	$(VideoDIV3).css({'width':videoDivWidth+'px','height':smallHeight+'px'});
	$(Videodiv1).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center','margin':'0 0 0' ,'float':'left','border':'1px solid black'});
	$(Videodiv1).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId1+'" autoplay="autoplay"></video>');
  $(Videodiv2).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0' ,'float':'left','border':'1px solid black'});
  $(Videodiv2).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId2+'" autoplay="autoplay"></video>');
  $(Videodiv3).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv3).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId3+'" autoplay="autoplay"></video>');
  $(Videodiv4).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv4).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId4+'" autoplay="autoplay"></video>');
  $(Videodiv5).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv5).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId5+'" autoplay="autoplay"></video>');
  $(Videodiv6).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv6).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId6+'" autoplay="autoplay"></video>');
  $(Videodiv7).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv7).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId7+'" autoplay="autoplay"></video>');
  $(Videodiv8).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  $(Videodiv8).html('<video width="'+smallWidth+'" height="'+smallHeight+'" id="'+VideoId8+'" autoplay="autoplay"></video>');
  $(Videodiv9).css({'width':smallWidth+'px', 'height':smallHeight+'px','background':'url(images/member.png) no-repeat #ffffff center center', 'margin':'0 0 0','float':'left','border':'1px solid black'});
  ...
  videoDiv.appendChild(VideoDIV1);
  videoDiv.appendChild(VideoDIV2);
  videoDiv.appendChild(VideoDIV3);
}
