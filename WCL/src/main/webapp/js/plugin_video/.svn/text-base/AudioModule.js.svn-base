(function(){
    com.webrtc.AudioModule = Object.subClass({
        ...
        initiateMessageCallbacks : function() {
            ...
            this.onLocalStreamSuccess = function(stream){
                ...
                {   //add by pq 前端显示主叫方等待界面（在获取本地音视频成功之后调用）
             	    createCallAudioWait(formatRechange(thi$.RemoteUserID), thi$.SessionID);  
                    thi$.setupCall();
                }
                else
                {
        ...
        receiveAnswer : function (offerSessionId, answerSenssionId, gAnswer) {
            ...
            $(document.getElementById(CallAudioPrefix + formatRechange(this.RemoteUserID) + "Wait")).remove();
            $(document.getElementById(CallAudioPrefix + formatRechange(this.RemoteUserID))).show();
            $(document.getElementById(formatRechange(this.RemoteUserID)+CallAudioPrefix+"activelist")).show();
        ...
        {
        	//视频发起方删除等待状态界面
        	if(document.getElementById(CallAudioPrefix + formatRechange(this.RemoteUserID) + "Wait")){
        		 $(document.getElementById(CallAudioPrefix + formatRechange(this.RemoteUserID) + "Wait")).remove();
        	}
        	//前端操作，创建错误页面 add by pq
        	createAudioErrorPage(this.RemoteUserID,error,this.SessionID);
