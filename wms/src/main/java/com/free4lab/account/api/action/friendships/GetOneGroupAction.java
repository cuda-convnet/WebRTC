...
import com.free4lab.account.api.action.friendships.OneGroup;;

public class GetOneGroupAction extends BaseAction{
	/**
	 * 获取一群组，得到其基本信息
	 */
	private static final long serialVersionUID = 1L;
//	private static final LogUtilManager lol = new LogUtilManager(GetOneGroupAction.class);
	private String result = "";
	private String message = "";
	private OneGroup onegroup = null;
	
	@Profiled(tag="GetOneGroupAction.execute")
...
//				lol.warn(userId, "用户级根据用户Id获取用户失败", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
				...
				if(myGroupList == null){
					...
					this.setResult("fail");
//					lol.warn(userId, "用户级权限不够，获取该分组信息失败", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
					return SUCCESS;
				}
			}
		}else{
...
//			lol.error(userId, "用户级获取用户Id错误", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		...
		onegroup = new OneGroup(group_id, myGroupList.getName(), isRoot);
		if(onegroup != null){
			this.setMessage("获取成功");
			this.setResult("success");
//			lol.info(userId, "用户级获取该分组信息成功", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		}else{
			this.setMessage("获取失败");
			this.setResult("fail");
//			lol.error(userId, "用户级获取该分组信息失败", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		}
    	return SUCCESS;
	}
	...
	public OneGroup getOnegroup() {
		return onegroup;
	}

	public void setOnegroup(OneGroup onegroup) {
