...
public class RootGroupAction extends BaseAction {

	/**
	 * 查找根组，返回根组所在的group_id，若不存在根组则新建根组
	 */
	private static final long serialVersionUID = 1L;
//	private static final LogUtilManager lol = new LogUtilManager(RootGroupAction.class);
	private String result = "";
	private String message = "";
	private Integer group_id = -1;
	
	@Profiled(tag="RootGroupAction.execute")
...
//			lol.error(userId, "用户级获取用户Id错误", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		...
		this.setGroup_id(UtilFriendManager.getRootGroupId(userId));
		if(group_id > 0){
			this.setMessage("根组获取成功");
			this.setResult("success");
//			lol.info(userId, "用户级获取根组成功", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		}else{
			this.setMessage("根组获取失败");
			this.setResult("fail");
//			lol.error(userId, "用户级获取根组失败", "好友", accessToken, AccountModule.getClientIdByUser(accessToken));
		}
